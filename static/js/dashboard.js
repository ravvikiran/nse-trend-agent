// NSE Trend Scanner - Dashboard JavaScript
...
function loadDashboardData() {
    ...
    ]).then(([dashboard, marketStatus, openTrades, performance]) => {
...
function updateMarketStatus(status) {
    ...
    const status_class = status.is_market_hours ? 'bg-success' : 'bg-secondary';
    statusBadge.className = 'badge ' + status_class;
    statusBadge.textContent = status.is_market_hours ? 'OPEN' : 'CLOSED';
    
    // Time to close
    if (status.is_market_hours) {
        const closeTime = new Date(status.market_close);
        const now = new Date(status.current_time);
        const timeLeft = closeTime - now;
        const hours = Math.floor(timeLeft / 3600000);
        const minutes = Math.floor((timeLeft % 3600000) / 60000);
        document.getElementById('time-to-close').textContent = `${hours}h ${minutes}m`;
    } else {
        document.getElementById('time-to-close').textContent = 'Market Closed';
    }
}
