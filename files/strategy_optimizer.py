...
class StrategyPerformanceTracker:
    ...
    - Capped adaptive filters with bidirectional adjustment
    """
    ...
    # Default (neutral) filter values — relaxation moves toward these
    FILTER_DEFAULTS = {
        'volume_ratio_min': 1.5,
        'rsi_max': 65,
        'atr_min': 0.5,
    ...
    def __init__(self, trade_journal, data_dir: str = DATA_DIR):
        ...
        self.strategy_weights = {'TREND': 0.5, 'VERC': 0.5, 'MTF': 0.5}
        ...
        self.weight_history = {'TREND': [], 'VERC': [], 'MTF': []}
    ...
    def get_strategy_stats(self, strategy: str, limit: int = 50) -> Dict[str, Any]:
        ...
        if not closed:
            return {
                'strategy': strategy, 'trades': 0, 'win_rate': 0,
                'avg_rr': 0, 'max_drawdown': 0, 'avg_holding_days': 0,
                'weighted_score': 0,
            }
    ...
    def _calculate_win_rate(self, trades: List[Dict]) -> float:
        ...
        if not closed:
            return 0
        return len([t for t in closed if t.get('outcome') == 'WIN']) / len(closed) * 100
    ...
    def get_all_strategy_stats(self) -> Dict[str, Dict[str, Any]]:
        return {s: self.get_strategy_stats(s) for s in ['TREND', 'VERC', 'MTF']}
    ...
    def _calculate_performance_score(self, stats: Dict[str, Any]) -> float:
        ...
        return round((win_rate * 0.5) + (avg_rr * 20 * 0.3) - (max_drawdown * 0.2), 2)
    ...
    def _calculate_proportional_adjustment(self, stats: Dict[str, Any], context_key: tuple) -> float:
        ...
        rr_bonus = 0.1 if avg_rr >= 2.0 else 0.05 if avg_rr >= 1.5 else -0.05 if avg_rr < 1.0 else 0
        base_adjustment = win_rate_adjustment + rr_bonus
        context_multiplier = self.context_weights.get(context_key, 1.0)
        return round(base_adjustment * context_multiplier, 3)
    ...
    def auto_optimize(self) -> Dict[str, Any]:
        ...
        for strategy in ['TREND', 'VERC', 'MTF']:
            ...
            new_weight = round(self._bound_weight(self._smooth_weight(current_weight, raw_weight)), 3)
            if abs(new_weight - current_weight) > 0.01:
                action = 'reduced' if new_weight < current_weight else 'increased'
                changes[strategy] = {
                    'action': action,
                    'from': current_weight,
                    'to': new_weight,
    ...
    # ------------------------------------------------------------------
    # Adaptive filters — bidirectional (tighten AND relax)
    # ------------------------------------------------------------------

    def adapt_filters(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Tighten filters based on issues detected in analysis."""
        ...
        for issue in issues:
            ...
            if 'dead_stock' in issue or 'low_volatility' in issue:
                ...
                new_value = max(self.FILTER_FLOORS.get('atr_min', 0.1), current - 0.1)
                if new_value != current:
    ...
    def adapt_filters_based_on_performance(self) -> Dict[str, Any]:
        ...
        if avg_win_rate > 60:
            # Relax each filter by half the distance to its default
            for key, default_val in self.FILTER_DEFAULTS.items():
                ...
                if key == 'rsi_max':
                    # rsi_max relaxes upward toward default
                    ...
                    # Other filters relax downward toward default
                    if current > default_val:
    ...
    def adapt_filters_from_failure_patterns(self) -> Dict[str, Any]:
        ...
        if avg_rsi > 65:
            ...
            if new_value != current:
                self.adaptive_filters['rsi_max'] = new_value
                changes.append(f"rsi_max: {current} → {new_value} (avg_rsi={avg_rsi})")
        ...
        if avg_volume < 1.8:
            ...
            if new_value != current:
                self.adaptive_filters['volume_ratio_min'] = new_value
                changes.append(f"volume_ratio_min: {current} → {new_value} (avg_vol={avg_volume})")
        ...
        if avg_breakout < 0.05:
            ...
            if new_value != current:
                self.adaptive_filters['breakout_strength_min'] = new_value
                changes.append(f"breakout_strength_min: {current} → {new_value}")

        if changes:
            self._save_filters()
            logger.info(f"Failure pattern adaptations: {changes}")

        # After tightening, check if we also need relaxation
        self.adapt_filters_based_on_performance()

        return self.adaptive_filters.copy()

    # ------------------------------------------------------------------
    # No-trade / signal quality checks
    # ------------------------------------------------------------------

    def check_no_trade_conditions(self, atr: float, wick_to_body_ratio: float,
                                  nifty_direction: str) -> Dict[str, Any]:
        reasons = []
    ...
    def check_signal_quality(self, score: float, volume_ratio: float,
                             breakout_strength: float) -> Dict[str, Any]:
        reasons = []
        if volume_ratio < self.adaptive_filters.get('volume_ratio_min', 1.5):
            reasons.append(
                f"Volume ratio below threshold ({volume_ratio:.2f} < "
                f"{self.adaptive_filters.get('volume_ratio_min', 1.5)})"
            )
        ...
        if breakout_min > 0 and breakout_value < breakout_min:
            reasons.append(
                f"Breakout strength below threshold ({breakout_value:.1f}% < {breakout_min:.1f}%)"
            )
    ...
    def get_failure_patterns(self) -> Dict[str, Any]:
        ...
        losses = [t for t in closed if t.get('outcome') in ['LOSS', 'TIMEOUT']]
        if not losses:
            return {'avg_rsi': 0, 'avg_volume_ratio': 0, 'avg_breakout_strength': 0, 'atr_avg': 0, 'loss_count': 0}
    ...
    def detect_market_condition(self, ema_20: float, ema_50: float, atr: float,
                                nifty_direction: Optional[str] = None) -> str:
        if ema_20 <= 0 or ema_50 <= 0:
            return self._current_market_condition
        spread = abs(ema_20 - ema_50) / ema_50
        condition = 'TRENDING' if (spread > 0.02 and atr > 1.0) else 'SIDEWAYS'
        if nifty_direction and nifty_direction == 'BEARISH':
            condition = 'SIDEWAYS'
        self._current_market_condition = condition
        return condition
    ...
    def get_rank_score_formula(self) -> str:
        return "rank_score = (strategy_score * 0.6 + volume_score * 0.2 + breakout_score * 0.2)"

    def calculate_adaptive_rank_score(self, strategy_score: float, strategy_type: str,
                                      volume_ratio: float, breakout_strength: float) -> float:
        """Pure rank score — strategy weight is NOT mixed in here."""
    ...
    def _save_filters(self) -> None:
        ...
        data = {'filters': self.adaptive_filters, 'updated_at': datetime.now().isoformat()}
        try:
            with open(filepath, 'w') as f:
                json.dump(data, f, indent=2)
        except Exception as e:
    ...
    def configure_optimization(self, smoothing: Optional[float] = None,
    ...
    def get_status_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'market_condition': self._current_market_condition,
            'recency_bias': {'recent_weight': self.RECENT_WEIGHT, 'old_weight': self.OLD_WEIGHT},
...
class StrategyOptimizer:
    ...
    def evaluate(self):
        ...
        win_rate = (len(wins) / total * 100) if total > 0 else 0
        logger.info(f"Performance evaluation: {len(wins)} wins, win_rate: {win_rate:.1f}%")
        if win_rate < 40:
            logger.warning("Win rate below 40% — consider increasing filter strictness")
        elif win_rate > 60:
            logger.info("Win rate above 60% — filters may be relaxed slightly")
